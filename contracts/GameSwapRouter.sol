...
library GameSwapLibrary {
    using SafeMath for uint256;

    // returns sorted token addresses, used to handle return values from pairs sorted in this order
    function sortTokens(address tokenA, address tokenB) internal pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'GameSwapLibrary: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'GameSwapLibrary: ZERO_ADDRESS');
    }
    ...
    function pairFor(
        ...
        pair = address(IGameSwapFactory(factory).getPair(token0, token1));
    }
    ...
    function getReserves(
        ...
        (uint256 reserve0, uint256 reserve1, ) = IGameSwapPair(IGameSwapFactory(factory).getPair(tokenA, tokenB)).getReserves();
    ...
    function quote(
        ...
        require(amountA > 0, 'GameSwapLibrary: INSUFFICIENT_AMOUNT');
        require(reserveA > 0 && reserveB > 0, 'GameSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountOut(
        ...
        require(amountIn > 0, 'GameSwapLibrary: INSUFFICIENT_INPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'GameSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountIn(
        ...
        require(amountOut > 0, 'GameSwapLibrary: INSUFFICIENT_OUTPUT_AMOUNT');
        require(reserveIn > 0 && reserveOut > 0, 'GameSwapLibrary: INSUFFICIENT_LIQUIDITY');
    ...
    function getAmountsOut(
        ...
        require(path.length >= 2, 'GameSwapLibrary: INVALID_PATH');
...
contract GameSwapRouter is IGameSwapRouter {
    ...
    function _addLiquidity(
        ...
        if (IGameSwapFactory(factory).getPair(tokenA, tokenB) == address(0)) {
            IGameSwapFactory(factory).createPair(tokenA, tokenB);
        }
        (uint256 reserveA, uint256 reserveB) = GameSwapLibrary.getReserves(factory, tokenA, tokenB);
        ...
        )
    {
        ...
        address pair = GameSwapLibrary.pairFor(factory, tokenA, tokenB);
        ...
        )
    {
        ...
        address pair = GameSwapLibrary.pairFor(factory, token, WBNB);
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            (uint256 amount0Out, uint256 amount1Out) = input == token0
                ? (uint256(0), amountOut)
                : (amountOut, uint256(0));
            address to = i < path.length - 2 ? GameSwapLibrary.pairFor(factory, output, path[i + 2]) : _to;
            IGameSwapPair(GameSwapLibrary.pairFor(factory, input, output)).swap(amount0Out, amount1Out, to);
        }
    }
    ...
    function swapExactTokensForTokens(
        ...
        amounts = GameSwapLibrary.getAmountsOut(factory, amountIn, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'GameSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0],
            msg.sender,
            GameSwapLibrary.pairFor(factory, path[0], path[1]),
            amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = GameSwapLibrary.getAmountsIn(factory, amountOut, path);
    ...
    function swapExactBNBForTokens(
        ...
        require(path[0] == WBNB, 'GameSwapRouter: INVALID_PATH');
        amounts = GameSwapLibrary.getAmountsOut(factory, msg.value, path);
        require(amounts[amounts.length - 1] >= amountOutMin, 'GameSwapRouter: INSUFFICIENT_OUTPUT_AMOUNT');
        IWBNB(WBNB).deposit{value: amounts[0]}();
        assert(IWBNB(WBNB).transfer(GameSwapLibrary.pairFor(factory, path[0], path[1]), amounts[0]));
        _swap(amounts, path, to);
    }
    ...
    function swapTokensForExactBNB(
        ...
        require(path[path.length - 1] == WBNB, 'GameSwapRouter: INVALID_PATH');
        amounts = GameSwapLibrary.getAmountsIn(factory, amountOut, path);
        require(amounts[0] <= amountInMax, 'GameSwapRouter: EXCESSIVE_INPUT_AMOUNT');
        TransferHelper.safeTransferFrom(
            path[0],
    ...
    function _swapSupportingFeeOnTransferTokens(address[] memory path, address _to) internal virtual {
        for (uint256 i; i < path.length - 1; i++) {
            (address input, address output) = (path[i], path[i + 1]);
            (address token0, ) = GameSwapLibrary.sortTokens(input, output);
            IGameSwapPair pair = IGameSwapPair(GameSwapLibrary.pairFor(factory, input, output));
            uint256 amountInput;
    ...
    function swapExactBNBForTokensSupportingFeeOnTransferTokens(
        ...
        assert(IWBNB(WBNB).transfer(GameSwapLibrary.pairFor(factory, path[0], path[1]), amountIn));
        uint256 balanceBefore = IBEP20(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IBEP20(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
    ...
    {
        return GameSwapLibrary.getAmountsOut(factory, amountIn, path);
    }
    ...
    function getAmountsIn(uint256 amountOut, address[] memory path)
        ...
        returns (uint256[] memory amounts)
    {
        return GameSwapLibrary.getAmountsIn(factory, amountOut, path);
    }
}
